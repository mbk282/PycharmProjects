...
for song in songs:
    ...
    data = response.text  # raw code
   # text_data = data.decode('utf-8')  # raw code in text
    text = text_maker.handle(data)  # just text displayed on the website
    ...
    for word in lyrics_words:
        all_lyrics.append(word)


    response.close()
    print(ill_lyrics, len(ill_lyrics))
#IMPORT section
import collections



file_count = collections.Counter(all_lyrics) #the counter function
